    ...
    try:
        ...
        try:
            ...
            if not is_admin and not any(group in user_groups for group in config.OIDC_GROUP_NAME):
                # Revoke group memberships in the auth DB so a previously-authorized
                # user whose OIDC groups have been removed does not retain access.
                if username and user_module.has_user(username):
                    try:
                        user_module.update_user(username=username, group_names=[])
                    except Exception as revoke_err:
                        logger.warning(f"Could not revoke groups for unauthorized user {username}: {revoke_err}")
            ...
            # Create/update user — this must succeed before we proceed
            user_module.create_user(username=username, display_name=display_name, is_admin=is_admin)

            # Group and profile sync — non-fatal: log and continue so a transient
            # failure here does not orphan a newly-created user row.
            try:
                ...
                logger.warning(f"Group/profile sync failed for {username}, will retry on next login: {sync_err}")

            # Workspace detection (per D-07, D-08, WSOIDC-01/02/03)
            # Layered approach: plugin first, JWT claim fallback, then auto-assign
            # Non-fatal: workspace sync failures must not block login.
            try:
              ...
              logger.warning(f"Workspace sync failed for {username}, will retry on next login: {ws_err}")
