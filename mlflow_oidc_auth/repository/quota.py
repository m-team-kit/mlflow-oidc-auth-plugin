...
class QuotaRepository:
    ...
    def get_quota(self, username: str) -> Optional[SqlUserQuota]:
        with self._Session() as session:
            quota = self._get_or_none(session, username)
    ...
    def get_quota_by_user_id(self, user_id: int) -> Optional[SqlUserQuota]:
        with self._Session() as session:
            quota = session.query(SqlUserQuota).filter(SqlUserQuota.user_id == user_id).one_or_none()
            if quota is not None:
                session.expunge(quota)
            return quota
    ...
    def list_all_quotas(self) -> List[SqlUserQuota]:
        with self._Session() as session:
            quotas = session.query(SqlUserQuota).all()
            for q in quotas:
                session.expunge(q)
            return quotas
    ...
    def update_email(self, username: str, email: Optional[str]) -> None:
        with self._Session() as session:
            ...
            quota = session.query(SqlUserQuota).filter(SqlUserQuota.user_id == user.id).one_or_none()
            if quota is None:
                # Create a quota row to store the email. Leave quota_bytes as
                # None so enforcement falls back to the env default dynamically.
                quota = SqlUserQuota(
                    user_id=user.id,
                    quota_bytes=None,
                    used_bytes=0,
